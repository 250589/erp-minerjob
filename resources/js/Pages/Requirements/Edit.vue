...
const form = useForm({
  area_id:       props.requirement.area_id,
  justification: props.requirement.justification ?? '',
  required_date: props.requirement.required_date ?? null,
  items: props.requirement.items.map((item) => ({
    product_id:            item.product_id,
    unit_id:               item.unit_id,
    description:           item.description,
    quantity:              Number(item.quantity),
    estimated_unit_price:  item.estimated_unit_price ? Number(item.estimated_unit_price) : null,
  })),
})
...
function submit() {
  form.put(`/requirements/${props.requirement.id}`)
}
</script>
    ...
    <div class="d-flex align-center mb-6">
      ...
      <div>
        <div class="d-flex align-center ga-2">
          <h1 class="text-h5 font-weight-bold">Editar {{ requirement.code }}</h1>
        ...
        <v-col cols="12" md="4">
          <v-card variant="outlined" rounded="lg" class="mb-4">
            ...
            <v-list density="compact">
              <v-list-item>
                <template #title><span class="text-body-2">Total de ítems</span></template>
                <template #append>
                  <v-chip color="primary" size="small">{{ form.items.length }}</v-chip>
                </template>
              </v-list-item>
              <v-list-item>
                <template #title><span class="text-body-2">Costo estimado</span></template>
                <template #append>
