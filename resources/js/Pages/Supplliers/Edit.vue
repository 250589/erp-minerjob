...
const form = useForm({
  ...
  currency_default:  props.supplier.currency_default,
  payment_term_days: props.supplier.payment_term_days,
...
<template>
  <div>
    ...
    <div class="d-flex align-center mb-6">
      <v-btn icon="mdi-arrow-left" variant="text" :href="`/suppliers/${supplier.id}`" class="mr-2" />
      <div>
        <h1 class="text-h5 font-weight-bold">Editar Proveedor</h1>
        <p class="text-body-2 text-medium-emphasis">{{ supplier.business_name }} · {{ supplier.tax_id }}</p>
      </div>
    </div>
        ...
        <v-col cols="12" md="8">
          ...
          <v-card variant="outlined" rounded="lg" class="mb-4">
            ...
            <v-card-text class="pt-4">
              <v-row>
                <v-col cols="12">
                  <v-text-field v-model="form.business_name" label="Razón Social *"
                    variant="outlined" density="compact" :error-messages="form.errors.business_name" />
                </v-col>
                <v-col cols="12" md="6">
                  <v-text-field v-model="form.trade_name" label="Nombre Comercial"
                    variant="outlined" density="compact" :error-messages="form.errors.trade_name" />
                </v-col>
                <v-col cols="12" md="6">
                  <v-text-field v-model="form.tax_id" label="RUC *"
                    variant="outlined" density="compact" maxlength="20"
                    :error-messages="form.errors.tax_id" />
                </v-col>
                <v-col cols="12">
                  <v-text-field v-model="form.address" label="Dirección"
                    variant="outlined" density="compact" prepend-inner-icon="mdi-map-marker"
                ...
                <v-col cols="12" md="6">
                  <v-text-field v-model="form.email" label="Correo Electrónico"
                    type="email" variant="outlined" density="compact"
                    prepend-inner-icon="mdi-email" :error-messages="form.errors.email" />
                </v-col>
              </v-row>
            </v-card-text>
          </v-card>
          ...
          <v-card variant="outlined" rounded="lg" class="mb-4">
            ...
            <v-card-text class="pt-4">
              <v-row>
                <v-col cols="12" md="4">
                  <v-select v-model="form.currency_default" :items="currencies"
          ...
          <v-card variant="outlined" rounded="lg" class="mb-4">
            ...
            <v-card-text class="pt-4">
              <v-row>
                <v-col cols="12" md="5">
                  <v-text-field v-model="form.bank_name" label="Banco"
                    variant="outlined" density="compact" prepend-inner-icon="mdi-bank-outline" />
                </v-col>
                <v-col cols="12" md="7">
                  <v-text-field v-model="form.bank_account" label="Número de Cuenta / CCI"
                    variant="outlined" density="compact" prepend-inner-icon="mdi-credit-card-outline" />
                </v-col>
              </v-row>
            </v-card-text>
          </v-card>
          ...
          <v-card variant="outlined" rounded="lg">
            ...
            <div v-if="form.contacts.length === 0" class="text-center pa-6 text-medium-emphasis">
              <p class="text-body-2">Sin contactos.</p>
            </div>
            ...
            <div v-for="(contact, index) in form.contacts" :key="index">
              <v-card-text>
                ...
                <v-row dense>
                  <v-col cols="12" md="6">
                    <v-text-field v-model="form.contacts[index].name" label="Nombre *"
                      variant="outlined" density="compact"
                      :error-messages="form.errors[`contacts.${index}.name`]" />
                  </v-col>
                  <v-col cols="12" md="6">
                    <v-text-field v-model="form.contacts[index].position" label="Cargo"
                      variant="outlined" density="compact" />
                  </v-col>
                  <v-col cols="12" md="6">
                    <v-text-field v-model="form.contacts[index].phone" label="Teléfono"
                      variant="outlined" density="compact" prepend-inner-icon="mdi-phone" />
                  </v-col>
                  <v-col cols="12" md="6">
                    <v-text-field v-model="form.contacts[index].email" label="Correo"
                      type="email" variant="outlined" density="compact"
        ...
        <v-col cols="12" md="4">
          <v-card variant="outlined" rounded="lg">
            <v-card-text>
              ...
              <v-btn variant="tonal" block :href="`/suppliers/${supplier.id}`"
